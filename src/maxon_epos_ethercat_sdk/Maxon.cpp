...
bool Maxon::mapPdos(RxPdoTypeEnum rxPdoTypeEnum, TxPdoTypeEnum txPdoTypeEnum)
{
  ...
  switch (rxPdoTypeEnum)
  {
    ...
    case RxPdoTypeEnum::RxPdoCST:
      ...
      // Write objects...
      objectIndex = (OD_INDEX_TARGET_TORQUE << 16) | (0x00 << 8) | sizeof(int16_t) * 8;
      rxSuccess &=
          sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x01, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_OFFSET_TORQUE << 16) | (0x00 << 8) | sizeof(int16_t) * 8;
      rxSuccess &=
          sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x02, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      // Write number of objects
      rxSuccess &= sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x00, false, static_cast<uint8_t>(2),
                                  configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
    ...
    case RxPdoTypeEnum::RxPdoPVM:
      ...
      // Write objects...
      objectIndex = (OD_INDEX_CONTROLWORD << 16) | (0x00 << 8) | sizeof(int16_t) * 8;
      rxSuccess &=
          sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x01, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_TARGET_VELOCITY << 16) | (0x00 << 8) | sizeof(int32_t) * 8;
      rxSuccess &=
          sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x02, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_PROFILE_ACCELERATION << 16) | (0x00 << 8) | sizeof(uint32_t) * 8;
      rxSuccess &=
          sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x03, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_PROFILE_DECELERATION << 16) | (0x00 << 8) | sizeof(uint32_t) * 8;
      rxSuccess &=
          sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x04, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_MOTION_PROFILE_TYPE << 16) | (0x00 << 8) | sizeof(int16_t) * 8;
      rxSuccess &=
          sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x05, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      // Write number of objects
      rxSuccess &= sdoVerifyWrite(OD_INDEX_RX_PDO_MAPPING_3, 0x00, false, static_cast<uint8_t>(5),
                                  configuration_.configRunSdoVerifyTimeout);
      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
  ...
  switch (txPdoTypeEnum)
  {
    ...
    case TxPdoTypeEnum::TxPdoCST:
      ...
      // Write objects...
      objectIndex = (OD_INDEX_POSITION_ACTUAL << 16) | (0x00 << 8) | sizeof(int32_t) * 8;
      txSuccess &=
          sdoVerifyWrite(OD_INDEX_TX_PDO_MAPPING_3, 0x01, false, objectIndex, configuration_.configRunSdoVerifyTimeout);

      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_TORQUE_ACTUAL << 16) | (0x00 << 8) | sizeof(int16_t) * 8;
      txSuccess &=
          sdoVerifyWrite(OD_INDEX_TX_PDO_MAPPING_3, 0x02, false, objectIndex, configuration_.configRunSdoVerifyTimeout);

      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_VELOCITY_ACTUAL << 16) | (0x00 << 8) | sizeof(int32_t) * 8;
      txSuccess &=
          sdoVerifyWrite(OD_INDEX_TX_PDO_MAPPING_3, 0x03, false, objectIndex, configuration_.configRunSdoVerifyTimeout);

      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      // Write number of objects
      txSuccess &= sdoVerifyWrite(OD_INDEX_TX_PDO_MAPPING_3, 0x00, false, static_cast<uint8_t>(3),
                                  configuration_.configRunSdoVerifyTimeout);
    ...
    case TxPdoTypeEnum::TxPdoPVM:
      ...
      // Write objects...
      objectIndex = (OD_INDEX_VELOCITY_DEMAND << 16) | (0x00 << 8) | sizeof(int32_t) * 8;
      txSuccess &=
          sdoVerifyWrite(OD_INDEX_TX_PDO_MAPPING_3, 0x01, false, objectIndex, configuration_.configRunSdoVerifyTimeout);

      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      objectIndex = (OD_INDEX_STATUSWORD << 16) | (0x00 << 8) | sizeof(uint16_t) * 8;
      txSuccess &=
          sdoVerifyWrite(OD_INDEX_TX_PDO_MAPPING_3, 0x02, false, objectIndex, configuration_.configRunSdoVerifyTimeout);
      ...
      // Write number of objects
      txSuccess &= sdoVerifyWrite(OD_INDEX_TX_PDO_MAPPING_3, 0x00, false, static_cast<uint8_t>(2),
                                  configuration_.configRunSdoVerifyTimeout);
...
bool Maxon::configParam(ModeOfOperationEnum modeOfOperationEnum)
{
  ...
  switch (modeOfOperationEnum)
  {
    ...
    case ModeOfOperationEnum::CyclicSynchronousTorqueMode:
      ...
      configSuccess &= sdoVerifyWrite(OD_INDEX_MOTOR_DATA, 0x01, false, nominalCurrent);

      std::this_thread::sleep_for(std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));

      torqueConstant = static_cast<uint32_t>(1000000.0 * configuration_.torqueConstantNmA);
      configSuccess &= sdoVerifyWrite(OD_INDEX_MOTOR_DATA, 0x05, false, torqueConstant);
