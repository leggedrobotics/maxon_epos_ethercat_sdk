...
bool Maxon::startup() {
  ...
  //   // success &= sendSdoRead(OD_INDEX_MOTOR_RATED_CURRENT, 0, false,
  //   // motorRatedCurrent); update the configuration to accomodate the new motor
  //   // rated current value
  //   configuration_.motorRatedCurrentA =
  //       static_cast<double>(motorRatedCurrent) / 1000.0;
...
bool Maxon::mapPdos(RxPdoTypeEnum rxPdoTypeEnum, TxPdoTypeEnum txPdoTypeEnum) {
  ...
  switch (txPdoTypeEnum) {
    case TxPdoTypeEnum::TxPdoStandard:
      ...
      //                             static_cast<uint16_t>(0x1a1d),
      //                             configuration_.configRunSdoVerifyTimeout);
      // std::this_thread::sleep_for(
      //     std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
      // txSuccess &= sdoVerifyWrite(OD_INDEX_TX_PDO_ASSIGNMENT, 3, false,
      //                             static_cast<uint16_t>(0x1a1f),
      //                             configuration_.configRunSdoVerifyTimeout);
      // std::this_thread::sleep_for(
      //     std::chrono::microseconds(configuration_.configRunSdoVerifyTimeout));
